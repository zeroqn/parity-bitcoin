...
#[derive(Debug, PartialEq)]
pub struct TransactionOutputWithPhone {
    /// Receiver' phone
    pub phone: String,
    /// Amount in BTC
    pub amount: f64,
...
pub enum TransactionOutput {
    ...
    Phone(TransactionOutputWithPhone),
}
        ...
        S: Serializer,
    {
        ...
        for output in &self.outputs {
            match output {
                ...
                &TransactionOutput::Phone(ref phone_output) => {
                    state.serialize_entry(
                        &("phone".to_string() + phone_output.phone.as_str()),
                        &phone_output.amount,
                    )?;
                }
            }
        ...
        impl<'b> Visitor<'b> for TransactionOutputsVisitor {
                ...
                V: MapAccess<'b>,
            {
                ...
                while let Some(key) = try!(visitor.next_key::<String>()) {
                    ...
                    } else if key.starts_with("phone:") {
                        let mut key = key.to_string();
                        let phone = key.split_off(6);
                        let amount: f64 = visitor.next_value()?;
                        outputs.push(TransactionOutput::Phone(TransactionOutputWithPhone {
                            phone,
                            amount,
                        }));
                    } else {
...
mod tests {
    ...
    fn transaction_outputs_serialize() {
        let txout = TransactionOutputs {
            outputs: vec![
                TransactionOutput::Phone(TransactionOutputWithPhone {
                    phone: "12343432".into(),
                    amount: 0.01,
                }),
        ...
        assert_eq!(serde_json::to_string(&txout).unwrap(), r#"{"phone12343432":0.01,"1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa":123.45,"1H5m1XzvHsjWX3wwU781ubctznEpNACrNC":67.89,"data":"01020304","data":"05060708"}"#);
    }
